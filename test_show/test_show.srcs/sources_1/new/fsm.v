...
begin
	case(state)
		...
		buy1:
		begin 
			sum <= money[0]==0 ? sum-money[1] : sum-money[1]-1;
			sum_frac <= money[0]==0 ? 0 : 10 - money[0];
			num[0] <= num[0] - 1;
			counter <= 0;
		end
		...
		buy2:
		begin 
			sum <= money[2]==0 ? sum-money[3] : sum-money[3]-1;
			sum_frac <= money[2]==0 ? 0 : 10 - money[2];
			num[1] <= num[1] - 1;
			counter <= 0;
		end
...
begin
	case(state)
		...
		begin
			...
			charge_ind <= 0;
			admin_ind <= 0;
			if(sum > money[1] || (sum == money[1] && sum_frac >= money[0]))
		...
		begin
			...
			charge_ind <= 0;
			admin_ind <= 0;
			if(sum > money[3] || (sum == money[3] && sum_frac >= money[2]))
